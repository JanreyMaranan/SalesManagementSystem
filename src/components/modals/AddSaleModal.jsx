...
function AddSaleModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { data: last } = await supabase.from('sales').select('transno').order('transno', { ascending: false }).limit(1)
    const lastNum = last && last.length > 0 ? parseInt(last[0].transno.replace('TR', '')) : 0
    const transno = `TR${String(lastNum + 1).padStart(6, '0')}`

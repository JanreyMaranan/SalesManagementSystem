...
function AddLineItemModal({ transNo, onClose }) {
  ...
  useEffect(() => {
    if (form.prodCode) {
      const price = dummyPrices[form.prodCode] || ''
  ...
  const handleSubmit = (e) => {
    ...
    setError('')
    alert(`Line item added to ${transNo}! (will connect to Supabase later)`)
    onClose()
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-700">
            Add Line Item <span className="text-blue-600 text-sm">— {transNo}</span>
          </h3>
